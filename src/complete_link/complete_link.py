...
while(len(euclideanDistances) != 1):
    ...
    for i in range(len(euclideanDistances)):
        if i == mininumDistance[1]:
            ...
            newRow.append(max(euclideanDistances[mininumDistance[1]][i], euclideanDistances[mininumDistance[2]][i]))
...
plot_dataset(dataframe, np.array(resultDataframe['cluster']), suptitle="complete_link")

outputFile = dataset[:-3] + 'clu'
resultDataframe.to_csv(f'../out/complete_link_{outputFile}', sep=' ', index=False, header=False)  